...
2. 如何运行调度程序来重现本章中的每个实例？
   ...
   # -S 用来模拟进程在时间片即将用完之前，发出一个I/O（让出CPU），但在它恢复执行时（Ready状态），该进程还在原优先级队列中，而且它的时间片完全没有改变。如果配合-a使用，则在时间配额即将用完之前，发出一个I/O（让出CPU），在它恢复执行时保持原队列优先级。
   ...
   # JOB0 一开始运行3个时间配额后，被移入到最低优先级队列中，然后开始运行JOB1和JOB2，JOB0则一直处于饥饿状态，只有等待JOB1和JOB2运行结束，才会继续运行JOB0。假设JOB1和JOB2无限执行状态，那JOB0将会被饿死。
   ...
   # 另外，在有时间配额（-a）、没有-S 、-B的条件下，低优先级队列中的进程还是会处于饥饿状态且被饿死。
